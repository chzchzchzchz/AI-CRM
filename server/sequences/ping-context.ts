...
export const PING_PLATFORM_CONTEXT = {
  ...
  integration: {
    ...
    description: "Deploy the company as a high-assurance IoP or MFA factor within PingOne DaVinci flows. Enforce strict device verification standards as a prerequisite for access.",
    benefits: [
  ...
  customerExample: {
    ...
    solution: "the company + Ping integration replaced VPN with device-bound identity",
    outcomes: [
  ...
  competitive: {
    vsOkta: "the company + Ping provides superior device trust and risk signals compared to Okta's native capabilities",
    vsAzure: "the company + Ping offers passwordless MFA at scale with better device compliance monitoring than Azure AD",
    vsSilverfort: "the company complements Ping with device-level risk signals; Silverfort focuses on identity fabric integration",
  },
  ...
  talkingPoints: {
    ...
    iam: [
      "Deploy the company as a high-assurance IoP or MFA factor within PingOne",
...
export function getPingAccountSummaryPrompt(accountData: {
  ...
  return `You are an elite Enterprise Account Executive for the company, a passwordless MFA/Zero Trust security company.
...
YOUR TASK:
Generate a comprehensive account brief that positions the company as the "Verified Trust Layer" for their Ping deployment.

REQUIREMENTS:
1. Analyze how the company complements their Ping infrastructure
2. Identify specific pain points that the company solves for Ping customers
3. Reference the Ping + the company integration approach (Orchestrated Defense)
...
- the company FIT: How the company complements their Ping deployment
- PAIN POINTS: Specific challenges for Ping customers that the company solves
...
export function getPingContactSummaryPrompt(contactData: {
  ...
  return `You are an elite Enterprise Account Executive for the company.
...
YOUR TASK:
Generate a personalized contact brief that references their Ping infrastructure and positions the company as the ideal complement.
...
3. Reference the Ping + the company integration approach
...
- BEST ANGLE: How to approach them with the company + Ping value prop
...
YOUR TASK:
Write a short (3-5 sentences), highly personalized outreach email that positions the company as the "Verified Trust Layer" for their Ping deployment.

REQUIREMENTS:
1. Reference their Ping infrastructure specifically
2. Highlight how the company complements Ping (Orchestrated Defense approach)
3. Reference one specific pain point that the company solves
...
GOOD EXAMPLES:
- "Given your Ping deployment, you've likely seen the challenge of balancing security with user experience. the company eliminates that tradeoff by adding device-bound passwordless authentication to your Ping flows—50% faster logins with 80% fewer identity attacks."
...
- "Ping gives you strong identity controls, but password resets still consume IT resources. the company eliminates password reset calls entirely by enabling passwordless authentication within PingOne DaVinci."
