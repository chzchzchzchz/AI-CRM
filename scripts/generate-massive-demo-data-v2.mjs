...
async function main() {
  ...
  for (let i = 0; i < 50; i++) {
    ...
    if ((i + 1) % 10 === 0) {
      console.log(`  ✓ Created ${i + 1} accounts...`);
    }
  }
  console.log(`  ✓ Total: 50 accounts created`);

  // Get all account IDs
  const [accountRows] = await db.execute("SELECT id, name, domain FROM accounts");
  const accounts = accountRows;
  console.log(`  ✓ Retrieved ${accounts.length} account IDs`);
  ...
  for (let i = 0; i < 200; i++) {
    ...
    await db.execute(
      ...
      [account.id, firstName, lastName, name, title, email, phone, linkedin]
    );
  ...
  // Get all contact IDs
  const [contactRows] = await db.execute("SELECT id, accountId FROM contacts");
...
[04:20] Sales Rep: All our crypto is FIPS 140-2 Level 2 validated...`
  ...
  for (let i = 0; i < 50; i++) {
    ...
    const contact = accountContacts.length > 0 ? randomElement(accountContacts) : null;
    ...
    const durationSeconds = durationMinutes * 60 + randomInt(0, 59); // Convert to total seconds
    ...
    await db.execute(
      ...
      [account.id, contact ? contact.id : null, title, durationSeconds, callDate, transcript]
    );
