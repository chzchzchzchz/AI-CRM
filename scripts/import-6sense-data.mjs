...
const DATA_AS_OF = '2025-12-15 00:00:00';
...
async function importData() {
  ...
  try {
    ...
    for (const row of buyingStageData) {
      await conn.execute(
        `INSERT INTO sixsenseBuyingStageMetrics (timeframe, buyingStage, numberOfAccounts, dataAsOf) VALUES (?, ?, ?, ?)`,
        [row.timeframe, row.buyingStage, row.numberOfAccounts, DATA_AS_OF]
      );
    }
    ...
    for (const row of engagementData) {
      await conn.execute(
        `INSERT INTO sixsenseEngagementMetrics (timeWindow, engagementState, accounts, dataAsOf) VALUES (?, ?, ?, ?)`,
        [row.timeWindow, row.engagementState, row.accounts, DATA_AS_OF]
      );
    }
    ...
    for (const row of keywordsData) {
      await conn.execute(
        `INSERT INTO sixsenseKeywords (keyword, totalAccounts, accountsWithWebVisits, accountsWith6QA, accountsWithOpportunities, accountsWithRelevantOpportunities, category, dataAsOf) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
        [row.keyword, row.totalAccounts, row.accountsWithWebVisits, row.accountsWith6QA, row.accountsWithOpportunities, row.accountsWithRelevantOpportunities, row.category, DATA_AS_OF]
      );
    }
    ...
    for (const row of performanceData) {
      await conn.execute(
        `INSERT INTO sixsense6QAPerformance (day, total6QAs, new6QAs, worked, unworked, avgSalesActivities, avgContactsReached, avgDaysToFirstActivity, avgDaysSinceLastActivity, dataAsOf) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        [row.day, row.total6QAs, row.new6QAs, row.worked, row.unworked, row.avgSalesActivities, row.avgContactsReached, row.avgDaysToFirstActivity, row.avgDaysSinceLastActivity, DATA_AS_OF]
      );
    }
