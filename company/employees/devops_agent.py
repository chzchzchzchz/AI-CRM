...
AGENT_NAME = "DevOps"
AGENT_ROLE = "Deployment, CI/CD, infrastructure"
...
def read_role():
    if not EMPLOYEE_ROLES_PATH.exists():
        return "DevOps role not found"
    with open(EMPLOYEE_ROLES_PATH) as f:
        content = f.read()
    start = content.find("### DevOps")
    if start == -1:
        return content[:500]
...
def pick_up_task():
    ...
    if not my_tasks:
        my_tasks = [t for t in board.get("ready", []) 
                    ...
                    (t.get("department") == "engineering" or "deploy" in t.get("title", "").lower() or "ci" in t.get("title", "").lower())]
    if my_tasks:
        task = my_tasks[0]
