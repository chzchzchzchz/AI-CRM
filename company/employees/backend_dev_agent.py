#!/usr/bin/env python3
"""
Backend Developer Agent - tRPC routers, API integrations, data pipelines
Like gstack's plan-eng-review specialist for backend work
"""
import json
import os
...
AGENT_ROLE = "tRPC routers, API integrations, data pipelines"
...
def read_role():
    if not EMPLOYEE_ROLES_PATH.exists():
        return "Backend Developer role not found"
    with open(EMPLOYEE_ROLES_PATH) as f:
        content = f.read()
    start = content.find("### Backend Developer")
    if start == -1:
        return content[:500]
...
def pick_up_task():
    ...
    if not my_tasks:
        my_tasks = [t for t in board.get("ready", []) 
                    ...
                    (t.get("department") == "engineering" or "api" in t.get("title", "").lower() 
                     or "backend" in t.get("title", "").lower())]
    if my_tasks:
        task = my_tasks[0]
...
def do_work(task_id, task_title):
    print(f"[{AGENT_NAME}] Building: {task_title}")
    return True
