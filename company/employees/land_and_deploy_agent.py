#!/usr/bin/env python3
"""
Land-and-Deploy Agent - Production deployment, release management
Like gstack's land-and-deploy specialist
"""
import json
import os
...
AGENT_NAME = "Land-and-Deploy"
AGENT_ROLE = "Production deployment, release management"
...
def read_role():
    if not EMPLOYEE_ROLES_PATH.exists():
        return "Land-and-Deploy role not found"
    with open(EMPLOYEE_ROLES_PATH) as f:
        content = f.read()
    start = content.find("### Land-and-Deploy")
    if start == -1:
        return content[:500]
...
def pick_up_task():
    ...
    if not my_tasks:
        my_tasks = [t for t in board.get("ready", []) 
                    ...
                    (t.get("department") == "engineering" or "deploy" in t.get("title", "").lower() or "release" in t.get("title", "").lower())]
    if my_tasks:
        task = my_tasks[0]
...
def do_work(task_id, task_title):
    print(f"[{AGENT_NAME}] Deploying: {task_title}")
    if "deploy" in task_title.lower():
        print(f"[{AGENT_NAME}] Deploying to production...")
    elif "release" in task_title.lower():
        print(f"[{AGENT_NAME}] Managing release...")
    return True
