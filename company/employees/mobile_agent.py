#!/usr/bin/env python3
"""
Mobile Agent - iOS/Android apps, mobile UI
"""
import json
import os
...
AGENT_NAME = "Mobile"
AGENT_ROLE = "iOS/Android apps, mobile UI"
...
def read_role():
    if not EMPLOYEE_ROLES_PATH.exists():
        return "Mobile role not found"
    with open(EMPLOYEE_ROLES_PATH) as f:
        content = f.read()
    start = content.find("### Mobile")
    if start == -1:
        return content[:500]
...
def pick_up_task():
    ...
    if not my_tasks:
        my_tasks = [t for t in board.get("ready", []) 
                    ...
                    ("mobile" in t.get("title", "").lower() or "ios" in t.get("title", "").lower() or "android" in t.get("title", "").lower())]
    if my_tasks:
        task = my_tasks[0]
...
def do_work(task_id, task_title):
    print(f"[{AGENT_NAME}] Building mobile: {task_title}")
    return True
