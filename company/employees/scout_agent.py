#!/usr/bin/env python3
"""
 Scout Agent - First-look bug reports, triage
 Like gstack's scout specialist
 """
import json
import os
...
AGENT_NAME = "Scout"
AGENT_ROLE = "First-look bug reports, triage"
...
def read_role():
    if not EMPLOYEE_ROLES_PATH.exists():
        return "Scout role not found"
    with open(EMPLOYEE_ROLES_PATH) as f:
        content = f.read()
    start = content.find("### Scout")
    if start == -1:
        return content[:500]
...
def pick_up_task():
    ...
    if not my_tasks:
        my_tasks = [t for t in board.get("ready", []) 
                    ...
                    ("bug" in t.get("title", "").lower() or "triage" in t.get("title", "").lower())]
    if my_tasks:
        task = my_tasks[0]
...
def do_work(task_id, task_title):
    print(f"[{AGENT_NAME}] Triaging: {task_title}")
    if "bug" in task_title.lower():
        print(f"[{AGENT_NAME}] Reproducing bug...")
    elif "triage" in task_title.lower():
        print(f"[{AGENT_NAME}] Triaging issue...")
    return True
