#!/usr/bin/env python3
"""
Integration Agent - Third-party APIs, webhooks, sync
"""
import json
import os
...
AGENT_ROLE = "Third-party APIs, webhooks, sync"
...
def read_role():
    if not EMPLOYEE_ROLES_PATH.exists():
        return "Integration role not found"
    with open(EMPLOYEE_ROLES_PATH) as f:
        content = f.read()
    start = content.find("### Integration")
    if start == -1:
        return content[:500]
...
def pick_up_task():
    ...
    if not my_tasks:
        my_tasks = [t for t in board.get("ready", []) 
                    ...
                    ("integration" in t.get("title", "").lower() or "webhook" in t.get("title", "").lower() or "sync" in t.get("title", "").lower())]
    if my_tasks:
        task = my_tasks[0]
...
def do_work(task_id, task_title):
    print(f"[{AGENT_NAME}] Integrating: {task_title}")
    return True
