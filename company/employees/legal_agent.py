#!/usr/bin/env python3
"""
Legal Agent - Contracts, compliance, IP
"""
import json
import os
...
AGENT_NAME = "Legal"
AGENT_ROLE = "Contracts, compliance, IP"
...
def read_role():
    if not EMPLOYEE_ROLES_PATH.exists():
        return "Legal role not found"
    with open(EMPLOYEE_ROLES_PATH) as f:
        content = f.read()
    start = content.find("### Legal")
    if start == -1:
        return content[:500]
...
def pick_up_task():
    ...
    if not my_tasks:
        my_tasks = [t for t in board.get("ready", []) 
                    ...
                    ("legal" in t.get("title", "").lower() or "contract" in t.get("title", "").lower() or "compliance" in t.get("title", "").lower())]
    if my_tasks:
        task = my_tasks[0]
...
def do_work(task_id, task_title):
    print(f"[{AGENT_NAME}] Handling: {task_title}")
    return True
