#!/usr/bin/env python3
"""
Metrics Agent - Performance tracking, KPIs, dashboards
...
AGENT_NAME = "Metrics"
AGENT_ROLE = "Performance tracking, KPIs, dashboards"
...
def pick_up_task():
    ...
    if not my_tasks:
        my_tasks = [t for t in board.get("ready", []) 
                    ...
                    (t.get("department") == "product" or "metric" in t.get("title", "").lower() or "kpi" in t.get("title", "").lower() or "dashboard" in t.get("title", "").lower())]
    if my_tasks:
        task = my_tasks[0]
...
def do_work(task_id, task_title):
    print(f"[{AGENT_NAME}] Tracking: {task_title}")
    if "kpi" in task_title.lower():
        print(f"[{AGENT_NAME}] Calculating KPIs...")
    elif "dashboard" in task_title.lower():
        print(f"[{AGENT_NAME}] Building dashboard...")
    elif "metric" in task_title.lower():
        print(f"[{AGENT_NAME}] Tracking metrics...")
    return True
