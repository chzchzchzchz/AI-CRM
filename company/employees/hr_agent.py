#!/usr/bin/env python3
"""
HR Agent - Hiring, onboarding, culture
"""
import json
import os
...
AGENT_NAME = "HR"
AGENT_ROLE = "Hiring, onboarding, culture"
...
def pick_up_task():
    ...
    if not my_tasks:
        my_tasks = [t for t in board.get("ready", []) 
                    ...
                    ("hire" in t.get("title", "").lower() or "onboard" in t.get("title", "").lower() or "culture" in t.get("title", "").lower())]
    if my_tasks:
        task = my_tasks[0]
...
def do_work(task_id, task_title):
    print(f"[{AGENT_NAME}] Managing: {task_title}")
    return True
