#!/usr/bin/env python3
"""
Performance Agent - Baseline page load times, Core Web Vitals
Like gstack's benchmark specialist
"""
import json
import os
...
AGENT_ROLE = "Baseline page load times, Core Web Vitals, resource sizes"
...
def read_role():
    if not EMPLOYEE_ROLES_PATH.exists():
        return "Performance role not found"
    with open(EMPLOYEE_ROLES_PATH) as f:
        content = f.read()
    start = content.find("### Performance")
    if start == -1:
        return content[:500]
...
def pick_up_task():
    ...
    if not my_tasks:
        my_tasks = [t for t in board.get("ready", []) 
                    ...
                    (t.get("department") == "engineering" or "perform" in t.get("title", "").lower())]
    if my_tasks:
        task = my_tasks[0]
...
def do_work(task_id, task_title):
    print(f"[{AGENT_NAME}] Benchmarking: {task_title}")
    print(f"[{AGENT_NAME}] Measuring page load...")
    print(f"[{AGENT_NAME}] Checking Core Web Vitals...")
    return True
