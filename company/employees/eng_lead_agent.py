#!/usr/bin/env python3
"""
Engineering Lead Agent - Technical architecture, system design, code quality
...
AGENT_ROLE = "Technical architecture, system design, code quality"
...
def read_role():
    if not EMPLOYEE_ROLES_PATH.exists():
        return "Engineering Lead role not found"
    with open(EMPLOYEE_ROLES_PATH) as f:
        content = f.read()
    start = content.find("### Engineering Lead")
    if start == -1:
        return content[:500]
