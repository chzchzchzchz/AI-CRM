#!/usr/bin/env python3
"""
Sales Lead Agent - Customer acquisition, demos, onboarding
...
AGENT_ROLE = "Customer acquisition, demos, onboarding"
...
def read_role():
    if not EMPLOYEE_ROLES_PATH.exists():
        return "Sales Lead role not found"
    with open(EMPLOYEE_ROLES_PATH) as f:
        content = f.read()
    start = content.find("### Sales Lead")
    if start == -1:
        return content[:500]
