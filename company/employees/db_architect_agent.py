#!/usr/bin/env python3
"""
Database Architect Agent - Schema design, migrations, query optimization
Like gstack's plan-eng-review specialist
"""
import json
import os
...
AGENT_ROLE = "Database schema, migrations, query optimization, multi-tenant"
...
def read_role():
    if not EMPLOYEE_ROLES_PATH.exists():
        return "DB Architect role not found"
    with open(EMPLOYEE_ROLES_PATH) as f:
        content = f.read()
    start = content.find("### Database Architect")
    if start == -1:
        return content[:500]
...
def pick_up_task():
    ...
    if not my_tasks:
        my_tasks = [t for t in board.get("ready", []) 
                    ...
                    (t.get("department") == "engineering" or "db" in t.get("title", "").lower() or "schema" in t.get("title", "").lower())]
    if my_tasks:
        task = my_tasks[0]
...
def do_work(task_id, task_title):
    ...
    if "schema" in task_title.lower():
        print(f"[{AGENT_NAME}] Designing schema...")
    elif "migration" in task_title.lower():
        print(f"[{AGENT_NAME}] Running migration...")
    elif "query" in task_title.lower():
        print(f"[{AGENT_NAME}] Optimizing query...")
    return True
