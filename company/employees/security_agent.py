#!/usr/bin/env python3
"""
Security Agent - OWASP Top 10, STRIDE, compliance
...
AGENT_NAME = "Security"
AGENT_ROLE = "OWASP Top 10, STRIDE, compliance"
...
def read_role():
    if not EMPLOYEE_ROLES_PATH.exists():
        return "Security role not found"
    with open(EMPLOYEE_ROLES_PATH) as f:
        content = f.read()
    start = content.find("### Security")
    if start == -1:
        return content[:500]
...
def pick_up_task():
    ...
    if not my_tasks:
        my_tasks = [t for t in board.get("ready", []) 
                    ...
                    (t.get("department") == "engineering" or "secur" in t.get("title", "").lower())]
    if my_tasks:
        task = my_tasks[0]
...
def do_work(task_id, task_title):
    print(f"[{AGENT_NAME}] Scanning: {task_title}")
    if "secur" in task_title.lower():
        print(f"[{AGENT_NAME}] Running OWASP scan...")
    elif "compliance" in task_title.lower():
        print(f"[{AGENT_NAME}] Checking compliance...")
    else:
        print(f"[{AGENT_NAME}] Auditing...")
    return True
