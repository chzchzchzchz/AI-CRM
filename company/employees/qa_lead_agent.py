#!/usr/bin/env python3
"""
QA Lead Agent - Testing, bug reports, release validation
Like gstack's qa specialist
"""
import json
import os
...
AGENT_NAME = "QA Lead"
AGENT_ROLE = "Testing, bug reports, release validation"
...
def read_role():
    if not EMPLOYEE_ROLES_PATH.exists():
        return "QA Lead role not found"
    with open(EMPLOYEE_ROLES_PATH) as f:
        content = f.read()
    start = content.find("### QA Lead")
    if start == -1:
        return content[:500]
...
def pick_up_task():
    ...
    if not my_tasks:
        my_tasks = [t for t in board.get("ready", []) 
                    ...
                    (t.get("department") == "engineering" or "test" in t.get("title", "").lower())]
    if my_tasks:
        task = my_tasks[0]
...
def do_work(task_id, task_title):
    print(f"[{AGENT_NAME}] Testing: {task_title}")
    return True
