#!/usr/bin/env python3
"""
Data Analyst Agent - Market research, competitive analysis, reporting
Like gstack's metrics specialist
"""
import json
import os
...
AGENT_ROLE = "Market research, competitive analysis, reporting"
...
def read_role():
    if not EMPLOYEE_ROLES_PATH.exists():
        return "Data Analyst role not found"
    with open(EMPLOYEE_ROLES_PATH) as f:
        content = f.read()
    start = content.find("### Data Analyst")
    if start == -1:
        return content[:500]
...
def pick_up_task():
    ...
    if not my_tasks:
        my_tasks = [t for t in board.get("ready", []) 
                    ...
                    (t.get("department") == "product" or "market" in t.get("title", "").lower() or "research" in t.get("title", "").lower())]
    if my_tasks:
        task = my_tasks[0]
...
def do_work(task_id, task_title):
    print(f"[{AGENT_NAME}] Analyzing: {task_title}")
    if "market" in task_title.lower():
        print(f"[{AGENT_NAME}] Researching market...")
    elif "competitor" in task_title.lower():
        print(f"[{AGENT_NAME}] Analyzing competitors...")
    elif "report" in task_title.lower():
