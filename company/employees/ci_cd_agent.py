#!/usr/bin/env python3
"""
CI/CD Agent - Pipeline automation, build optimization
"""
import json
import os
...
AGENT_NAME = "CI/CD"
AGENT_ROLE = "Pipeline automation, build optimization"
...
def read_role():
    if not EMPLOYEE_ROLES_PATH.exists():
        return "CI/CD role not found"
    with open(EMPLOYEE_ROLES_PATH) as f:
        content = f.read()
    start = content.find("### CI/CD")
    if start == -1:
        return content[:500]
...
def pick_up_task():
    ...
    if not my_tasks:
        my_tasks = [t for t in board.get("ready", []) 
                    ...
                    ("pipeline" in t.get("title", "").lower() or "ci" in t.get("title", "").lower())]
    if my_tasks:
        task = my_tasks[0]
...
def do_work(task_id, task_title):
    print(f"[{AGENT_NAME}] Building pipeline: {task_title}")
    return True
