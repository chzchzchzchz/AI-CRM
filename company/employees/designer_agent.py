#!/usr/bin/env python3
"""
Designer Agent - UI/UX implementation, mockups, design system
...
AGENT_NAME = "Designer"
AGENT_ROLE = "UI/UX design, mockups, design system"
...
def read_role():
    if not EMPLOYEE_ROLES_PATH.exists():
        return "Designer role not found"
    with open(EMPLOYEE_ROLES_PATH) as f:
        content = f.read()
    start = content.find("### UI/UX Designer")
    if start == -1:
        return content[:500]
...
def pick_up_task():
    ...
    if not my_tasks:
        my_tasks = [t for t in board.get("ready", []) 
                    ...
                    (t.get("department") == "design" or "design" in t.get("title", "").lower())]
    if my_tasks:
        task = my_tasks[0]
