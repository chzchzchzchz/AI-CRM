#!/usr/bin/env python3
"""
Plan-CEO-Review Agent - Strategic review, CEO-level decisions
Like gstack's plan-ceo-review specialist
"""
import json
import os
...
AGENT_NAME = "Plan-CEO-Review"
AGENT_ROLE = "Strategic review, CEO-level decisions"
...
def read_role():
    if not EMPLOYEE_ROLES_PATH.exists():
        return "Plan-CEO-Review role not found"
    with open(EMPLOYEE_ROLES_PATH) as f:
        content = f.read()
    start = content.find("### Plan-CEO-Review")
    if start == -1:
        return content[:500]
...
def pick_up_task():
    ...
    if not my_tasks:
        my_tasks = [t for t in board.get("ready", []) 
                    ...
                    (t.get("department") == "leadership" or "strategic" in t.get("title", "").lower() or "ceo" in t.get("title", "").lower())]
    if my_tasks:
        task = my_tasks[0]
...
def do_work(task_id, task_title):
    print(f"[{AGENT_NAME}] Reviewing strategically: {task_title}")
    if "strategic" in task_title.lower():
        print(f"[{AGENT_NAME}] Strategic analysis...")
    elif "pivot" in task_title.lower():
        print(f"[{AGENT_NAME}] Evaluating pivot...")
    return True
