#!/usr/bin/env python3
"""
Release Agent - Sync main, run tests, push, open PR
Like gstack's ship specialist
"""
import json
import os
...
AGENT_NAME = "Release"
AGENT_ROLE = "Sync main, run tests, audit coverage, push, open PR"
...
def read_role():
    if not EMPLOYEE_ROLES_PATH.exists():
        return "Release role not found"
    with open(EMPLOYEE_ROLES_PATH) as f:
        content = f.read()
    start = content.find("### Release")
    if start == -1:
        return content[:500]
...
def pick_up_task():
    ...
    if not my_tasks:
        my_tasks = [t for t in board.get("ready", []) 
                    ...
                    (t.get("department") == "engineering" or "releas" in t.get("title", "").lower())]
    if my_tasks:
        task = my_tasks[0]
...
def do_work(task_id, task_title):
    print(f"[{AGENT_NAME}] Releasing: {task_title}")
    print(f"[{AGENT_NAME}] Syncing main branch...")
    print(f"[{AGENT_NAME}] Running tests...")
    print(f"[{AGENT_NAME}] Auditing coverage...")
    print(f"[{AGENT_NAME}] Pushing to remote...")
    print(f"[{AGENT_NAME}] Opening PR...")
    return True
