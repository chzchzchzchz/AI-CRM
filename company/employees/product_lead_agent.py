#!/usr/bin/env python3
"""
Product Lead Agent - Feature prioritization, user experience, roadmap
...
AGENT_ROLE = "Feature prioritization, user experience, roadmap"
...
def read_role():
    if not EMPLOYEE_ROLES_PATH.exists():
        return "Product Lead role not found"
    with open(EMPLOYEE_ROLES_PATH) as f:
        content = f.read()
    start = content.find("### Product Lead")
    if start == -1:
        return content[:500]
