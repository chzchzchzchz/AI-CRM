#!/usr/bin/env python3
"""
Office Hours Agent - Customer calls, demos, feedback
...
AGENT_NAME = "Office Hours"
AGENT_ROLE = "Customer calls, demos, feedback collection"
...
def read_role():
    if not EMPLOYEE_ROLES_PATH.exists():
        return "Office Hours role not found"
    with open(EMPLOYEE_ROLES_PATH) as f:
        content = f.read()
    start = content.find("### Office Hours")
    if start == -1:
        return content[:500]
...
def pick_up_task():
    ...
    if not my_tasks:
        my_tasks = [t for t in board.get("ready", []) 
                    ...
                    (t.get("department") == "sales" or "demo" in t.get("title", "").lower() or "call" in t.get("title", "").lower())]
    if my_tasks:
        task = my_tasks[0]
...
def do_work(task_id, task_title):
    print(f"[{AGENT_NAME}] On call: {task_title}")
    if "demo" in task_title.lower():
        print(f"[{AGENT_NAME}] Giving demo...")
    elif "call" in task_title.lower():
        print(f"[{AGENT_NAME}] Customer call...")
    elif "feedback" in task_title.lower():
        print(f"[{AGENT_NAME}] Collecting feedback...")
    return True
