#!/usr/bin/env python3
"""
Docs Agent - Documentation, API references, guides
"""
import json
import os
...
AGENT_NAME = "Docs"
AGENT_ROLE = "Documentation, API references, guides"
...
def read_role():
    if not EMPLOYEE_ROLES_PATH.exists():
        return "Docs role not found"
    with open(EMPLOYEE_ROLES_PATH) as f:
        content = f.read()
    start = content.find("### Docs")
    if start == -1:
        return content[:500]
...
def pick_up_task():
    ...
    if not my_tasks:
        my_tasks = [t for t in board.get("ready", []) 
                    ...
                    (t.get("department") == "product" or "doc" in t.get("title", "").lower())]
    if my_tasks:
        task = my_tasks[0]
...
def do_work(task_id, task_title):
    print(f"[{AGENT_NAME}] Documenting: {task_title}")
    return True
