#!/usr/bin/env python3
"""
 Design-Review Agent - UI/UX review, design system compliance
 Like gstack's design-review specialist
 """
import json
import os
...
AGENT_ROLE = "UI/UX review, design system compliance"
...
def read_role():
    if not EMPLOYEE_ROLES_PATH.exists():
        return "Design-Review role not found"
    with open(EMPLOYEE_ROLES_PATH) as f:
        content = f.read()
    start = content.find("### Design-Review")
    if start == -1:
        return content[:500]
...
def pick_up_task():
    ...
    if not my_tasks:
        my_tasks = [t for t in board.get("ready", []) 
                    ...
                    ("ui" in t.get("title", "").lower() or "design" in t.get("title", "").lower())]
    if my_tasks:
        task = my_tasks[0]
...
def do_work(task_id, task_title):
    print(f"[{AGENT_NAME}] Reviewing design: {task_title}")
    if "ui" in task_title.lower():
        print(f"[{AGENT_NAME}] Checking UI...")
    elif "design" in task_title.lower():
        print(f"[{AGENT_NAME}] Reviewing design system...")
    return True
