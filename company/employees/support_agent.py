#!/usr/bin/env python3
"""
Support Agent - Customer success, technical support, training
Like gstack's support specialist
"""
import json
import os
...
AGENT_NAME = "Support"
AGENT_ROLE = "Customer success, technical support, training"
...
def read_role():
    if not EMPLOYEE_ROLES_PATH.exists():
        return "Support role not found"
    with open(EMPLOYEE_ROLES_PATH) as f:
        content = f.read()
    start = content.find("### Support")
    if start == -1:
        return content[:500]
...
def do_work(task_id, task_title):
    print(f"[{AGENT_NAME}] Supporting: {task_title}")
    if "onboard" in task_title.lower():
        print(f"[{AGENT_NAME}] Onboarding customer...")
    elif "troubleshoot" in task_title.lower():
        print(f"[{AGENT_NAME}] Troubleshooting...")
    elif "doc" in task_title.lower():
        print(f"[{AGENT_NAME}] Writing help docs...")
    else:
        print(f"[{AGENT_NAME}] Executing support task...")
    return True
