#!/usr/bin/env python3
"""
Frontend Developer Agent - UI implementation, React components
...
AGENT_ROLE = "React components, UI/UX implementation"
...
def read_role():
    if not EMPLOYEE_ROLES_PATH.exists():
        return "Frontend Developer role not found"
    with open(EMPLOYEE_ROLES_PATH) as f:
        content = f.read()
    start = content.find("### Frontend Developer")
    if start == -1:
        return content[:500]
...
def pick_up_task():
    ...
    if not my_tasks:
        my_tasks = [t for t in board.get("ready", []) 
                    ...
                    (t.get("department") == "engineering" or "frontend" in t.get("title", "").lower())]
    if my_tasks:
        task = my_tasks[0]
