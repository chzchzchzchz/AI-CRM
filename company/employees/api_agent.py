#!/usr/bin/env python3
"""
API Agent - API design, documentation, testing
"""
import json
import os
...
AGENT_NAME = "API"
AGENT_ROLE = "API design, documentation, testing"
...
def read_role():
    if not EMPLOYEE_ROLES_PATH.exists():
        return "API role not found"
    with open(EMPLOYEE_ROLES_PATH) as f:
        content = f.read()
    start = content.find("### API")
    if start == -1:
        return content[:500]
...
def pick_up_task():
    ...
    if not my_tasks:
        my_tasks = [t for t in board.get("ready", []) 
                    ...
                    ("api" in t.get("title", "").lower() or "router" in t.get("title", "").lower())]
    if my_tasks:
        task = my_tasks[0]
...
def do_work(task_id, task_title):
    print(f"[{AGENT_NAME}] Building API: {task_title}")
    return True
