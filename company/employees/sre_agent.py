#!/usr/bin/env python3
"""
SRE Agent - Post-deploy monitoring, error tracking
...
AGENT_NAME = "SRE"
AGENT_ROLE = "Post-deploy monitoring, console errors, performance regressions"
...
def read_role():
    if not EMPLOYEE_ROLES_PATH.exists():
        return "SRE role not found"
    with open(EMPLOYEE_ROLES_PATH) as f:
        content = f.read()
    start = content.find("### SRE")
    if start == -1:
        return content[:500]
...
def pick_up_task():
    ...
    if not my_tasks:
        my_tasks = [t for t in board.get("ready", []) 
                    ...
                    (t.get("department") == "engineering" or "monitor" in t.get("title", "").lower())]
    if my_tasks:
        task = my_tasks[0]
...
def do_work(task_id, task_title):
    print(f"[{AGENT_NAME}] Monitoring: {task_title}")
    print(f"[{AGENT_NAME}] Checking console errors...")
    print(f"[{AGENT_NAME}] Checking performance...")
    return True
