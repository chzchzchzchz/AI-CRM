#!/usr/bin/env python3
"""
Sales-Ops Agent - Sales process, automation, tools
"""
import json
import os
...
AGENT_NAME = "Sales-Ops"
AGENT_ROLE = "Sales process, automation, tools"
...
def read_role():
    if not EMPLOYEE_ROLES_PATH.exists():
        return "Sales-Ops role not found"
    with open(EMPLOYEE_ROLES_PATH) as f:
        content = f.read()
    start = content.find("### Sales-Ops")
    if start == -1:
        return content[:500]
...
def pick_up_task():
    ...
    if not my_tasks:
        my_tasks = [t for t in board.get("ready", []) 
                    ...
                    (t.get("department") == "sales" or "automation" in t.get("title", "").lower())]
    if my_tasks:
        task = my_tasks[0]
...
def do_work(task_id, task_title):
    print(f"[{AGENT_NAME}] Automating: {task_title}")
    return True
