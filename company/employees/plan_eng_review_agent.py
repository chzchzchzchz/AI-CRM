#!/usr/bin/env python3
"""
 Plan-Eng-Review Agent - Architecture review, technical design
...
AGENT_NAME = "Plan-Eng-Review"
AGENT_ROLE = "Architecture review, technical design"
...
def read_role():
    if not EMPLOYEE_ROLES_PATH.exists():
        return "Plan-Eng-Review role not found"
    with open(EMPLOYEE_ROLES_PATH) as f:
        content = f.read()
    start = content.find("### Plan-Eng-Review")
    if start == -1:
        return content[:500]
...
def pick_up_task():
    ...
    if not my_tasks:
        my_tasks = [t for t in board.get("ready", []) 
                    ...
                    ("architect" in t.get("title", "").lower() or "design" in t.get("title", "").lower())]
    if my_tasks:
        task = my_tasks[0]
...
def do_work(task_id, task_title):
    print(f"[{AGENT_NAME}] Reviewing: {task_title}")
    if "architect" in task_title.lower():
        print(f"[{AGENT_NAME}] Reviewing architecture...")
    elif "design" in task_title.lower():
        print(f"[{AGENT_NAME}] Reviewing design...")
    return True
