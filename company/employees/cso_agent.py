#!/usr/bin/env python3
"""
CSO Agent - Chief Security Officer, compliance, risk
Like gstack's cso specialist
"""
import json
import os
...
AGENT_NAME = "CSO"
AGENT_ROLE = "Chief Security Officer, compliance, risk management"
...
def read_role():
    if not EMPLOYEE_ROLES_PATH.exists():
        return "CSO role not found"
    with open(EMPLOYEE_ROLES_PATH) as f:
        content = f.read()
    start = content.find("### CSO")
    if start == -1:
        return content[:500]
...
def pick_up_task():
    ...
    if not my_tasks:
        my_tasks = [t for t in board.get("ready", []) 
                    ...
                    (t.get("department") == "leadership" or "compliance" in t.get("title", "").lower() or "risk" in t.get("title", "").lower())]
    if my_tasks:
        task = my_tasks[0]
...
def do_work(task_id, task_title):
    print(f"[{AGENT_NAME}] Assessing: {task_title}")
    if "compliance" in task_title.lower():
        print(f"[{AGENT_NAME}] Checking compliance...")
    elif "risk" in task_title.lower():
        print(f"[{AGENT_NAME}] Assessing risk...")
    elif "security" in task_title.lower():
        print(f"[{AGENT_NAME}] Security review...")
    return True
