#!/usr/bin/env python3
"""
Canary Agent - Gradual rollouts, monitoring
Like gstack's canary specialist
"""
import json
import os
...
AGENT_NAME = "Canary"
AGENT_ROLE = "Gradual rollouts, feature flags, monitoring"
...
def read_role():
    if not EMPLOYEE_ROLES_PATH.exists():
        return "Canary role not found"
    with open(EMPLOYEE_ROLES_PATH) as f:
        content = f.read()
    start = content.find("### Canary")
    if start == -1:
        return content[:500]
...
def pick_up_task():
    ...
    if not my_tasks:
        my_tasks = [t for t in board.get("ready", []) 
                    ...
                    (t.get("department") == "engineering" or "canary" in t.get("title", "").lower() or "rollout" in t.get("title", "").lower() or "feature" in t.get("title", "").lower())]
    if my_tasks:
        task = my_tasks[0]
...
def do_work(task_id, task_title):
    print(f"[{AGENT_NAME}] Rolling out: {task_title}")
    if "canary" in task_title.lower():
        print(f"[{AGENT_NAME}] Canary deployment...")
    elif "feature" in task_title.lower():
        print(f"[{AGENT_NAME}] Feature flag check...")
    return True
